# 🚧 Upcoming Major Refactor: Multi-LLM, AG UI, and CopilotKit UI 🚧

**This project is being upgraded to a modern, multi-LLM coding assistant with a beautiful CopilotKit UI!**

- **Backend:** Python (FastAPI), supporting multiple LLMs (OpenAI, DeepSeek, Anthropic, etc.), streaming via AG UI protocol.
- **Frontend:** React with CopilotKit, providing a delightful, production-grade chat/assistant interface and model selector.
- **Goal:** Seamless, real-time, event-driven AI coding assistant with the best UI/UX and flexibility.
...
```mermaid
flowchart TD
    User["User"] -->|"Web UI"| CopilotKitUI["CopilotKit React UI (AG UI Protocol)"]
    CopilotKitUI -->|"AG UI JSON Events (SSE/WebSocket)"| FastAPI["FastAPI Backend (Python)"]
    FastAPI -->|"LLM API Calls"| LLMs["Multiple LLM Providers (OpenAI, DeepSeek, Anthropic, etc.)"]
```

- **Frontend:** CopilotKit React UI, speaks AG UI protocol, lets users chat and select models.
- **Backend:** FastAPI, streams AG UI events, routes requests to the selected LLM.
- **LLMs:** Any supported provider, easily extendable.

---

## 🚚 Migration Plan

1. **Build FastAPI backend** with multi-LLM support and AG UI protocol streaming.
2. **Build CopilotKit React frontend** for chat, model selection, and beautiful UI.
3. **Test end-to-end** with multiple LLMs and real user flows.
4. **Remove old CLI code** (deepseek-eng.py) after successful migration.
